...
// Variables reactivas para capturar los textos del formulario
const nombre = ref('')
const email = ref('')
const mensaje = ref('')

// Estados para controlar el flujo de la interfaz del chat
const enviando = ref(false)
const enviadoExito = ref(false)
const errorEnvio = ref(false)

// Tu URL real de Formspree perfectamente configurada
...
const enviarFormulario = async () => {
  ...
  try {
    ...
    if (respuesta.ok) {
      ...
      // Limpiamos los campos reactivos
      nombre.value = ''
      email.value = ''
      mensaje.value = ''
    } else {
...
<template>
  <section id="contacto" class="py-20 px-6 bg-[#f7f7f9] dark:bg-[#080808] transition-colors duration-300">
    ...
    <div class="max-w-5xl mx-auto flex flex-col md:flex-row border border-neutral-200 dark:border-white/10 rounded-3xl overflow-hidden shadow-xl dark:shadow-2xl transition-all duration-300">
      ...
      <div class="md:w-1/3 bg-white dark:bg-white/5 p-10 flex flex-col items-center justify-center text-center border-r border-neutral-200 dark:border-white/10 italic transition-colors duration-300">
        <div class="w-32 h-32 rounded-full border-4 border-purple-500/20 dark:border-purple-500/30 p-1 mb-6 flex items-center justify-center text-neutral-500 dark:text-gray-400 bg-neutral-100 dark:bg-black font-mono text-xs font-bold transition-colors duration-300 tracking-wider">
          AMS
        </div>
      ...
      <div class="md:w-2/3 bg-[#f0f0f4] dark:bg-black p-8 relative transition-colors duration-300 flex flex-col justify-between min-h-[400px]">
        ...
        <div>
          ...
          <div class="space-y-6">
            <div class="bg-white dark:bg-white/10 text-neutral-800 dark:text-white p-4 rounded-2xl rounded-tl-none max-w-xs text-sm italic border border-neutral-200 dark:border-white/5 shadow-sm transition-all duration-300">
              ¡Hola! Envíame un mensaje y te responderé lo antes posible.
            </div>

            <div v-if="enviadoExito" class="bg-purple-600 text-white p-4 rounded-2xl rounded-tl-none max-w-xs text-sm italic border border-purple-500/20 shadow-md transition-all">
              ¡Mensaje enviado con éxito! 🚀 Acabo de recibir tu notificación en mi correo. Te responderé lo antes posible. ¡Gracias!
            </div>
          </div>
        </div>

        <form v-if="!enviadoExito" class="space-y-4 mt-8" @submit.prevent="enviarFormulario">
          ...
          <p v-if="errorEnvio" class="text-xs text-red-500 font-mono font-bold uppercase italic">
          ...
          >
            <span v-if="enviando" class="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin"></span>
